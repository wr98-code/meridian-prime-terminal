/**
 * Networks.tsx — ZERØ MERIDIAN 2026 push133
...
const StatusDot = React.memo(({ status }: { status: ChainStats['status'] }) => {
  const color =
    status === 'online'   ? 'rgba(0,155,95,1)'  :
    status === 'degraded' ? 'rgba(195,125,0,1)'  :
                            'rgba(208,35,75,1)';
  const shadow =
    status === 'online'   ? '0 2px 6px rgba(0,155,95,0.25)'  :
    status === 'degraded' ? '0 0 6px rgba(251,191,36,0.7)'  :
                            '0 2px 6px rgba(208,35,75,0.25)';
  return (
    <motion.div
...
const LatencyBar = React.memo(({ ms }: { ms: number }) => {
  ...
  const color = ms < 500 ? 'rgba(0,155,95,1)' : ms < 1500 ? 'rgba(195,125,0,1)' : 'rgba(208,35,75,1)';
  return (
...
const ChainCard = React.memo(({ chain, isMobile }: { chain: ChainStats; isMobile: boolean }) => {
  const cardStyle = useMemo(() => Object.freeze({
    background: 'rgba(15,40,100,0.04)',
  ...
  const statusColor = chain.status === 'online' ? 'rgba(0,155,95,1)' : chain.status === 'degraded' ? 'rgba(251,191,36,0.8)' : 'rgba(208,35,75,0.85)';
...
const SkeletonCard = React.memo(({ i }: { i: number }) => (
  <motion.div
    style={{ background: 'rgba(15,40,100,0.04)', border: '1px solid rgba(255,255,255,1)', borderRadius: '14px', padding: '16px', height: '130px' }}
...
const SummaryBar = React.memo(({ chains }: { chains: ChainStats[] }) => {
  ...
  return (
    <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' as const }}>
      <div style={chipStyle('rgba(0,155,95,1)')}>
        <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'rgba(0,155,95,1)', display: 'inline-block' }} />
        {online} Online
      </div>
      {degraded > 0 && (
        <div style={chipStyle('rgba(195,125,0,1)')}>
          <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'rgba(195,125,0,1)', display: 'inline-block' }} />
          {degraded} Degraded
        </div>
      )}
      {offline > 0 && (
        <div style={chipStyle('rgba(208,35,75,1)')}>
          <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'rgba(208,35,75,1)', display: 'inline-block' }} />
          {offline} Offline
        </div>
      )}
    ...
    <div style={{ padding: isMobile ? '16px 12px' : '24px', maxWidth: '1100px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap' as const, gap: '12px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <button onClick={refetch} style={{ padding: '7px 16px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,1)', color: 'rgba(255,255,255,0.6)', fontFamily: FONT, fontSize: '12px', cursor: 'pointer' }}>
            ↺ Refresh
          </button>
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        {loading && chains.length === 0 ? (
          ...
          <motion.div key="err" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            style={{ padding: '32px 16px', textAlign: 'center' as const, fontFamily: FONT, fontSize: '14px', color: 'rgba(208,35,75,0.85)' }}>
            <div>⚠ {error}</div>
            <button onClick={refetch} style={{ marginTop: '12px', padding: '8px 20px', borderRadius: '8px', background: 'rgba(208,35,75,0.09)', border: '1px solid rgba(208,35,75,0.28)', color: 'rgba(208,35,75,1)', fontFamily: FONT, fontSize: '13px', cursor: 'pointer' }}>Retry</button>
          </motion.div>
        ) : (
