...
import PriceChartTile    from '@/components/tiles/PriceChartTile';
import OrderBookTile    from '@/components/tiles/OrderBookTile';
import FundingRateTile  from '@/components/tiles/FundingRateTile';
import LiquidationTile  from '@/components/tiles/LiquidationTile';
import HeatmapTile      from '@/components/tiles/HeatmapTile';
import AISignalTile     from '@/components/tiles/AISignalTile';
import NewsTickerTile   from '@/components/tiles/NewsTickerTile';
...
// ── Live Tiles Section ─────────────────────────────────────────────────────
const TilesSection = memo(function TilesSection() {
  return (
    <motion.div
      ...
      style={{ marginTop: 24 }}
    >
      <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--colors-textMuted, #6b7280)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 12, fontFamily: 'var(--font-family)' }}>
        LIVE TERMINALS
      </div>
      {/* Row 1: Chart + OrderBook */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 12, marginBottom: 12 }}>
        <PriceChartTile symbol="BTCUSDT" />
        <OrderBookTile  symbol="BTCUSDT" />
      </div>
      {/* Row 2: Funding + Liquidation + Heatmap */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 12, marginBottom: 12 }}>
        <FundingRateTile />
        <LiquidationTile />
        <HeatmapTile />
      </div>
      {/* Row 3: AI Signals + News */}
